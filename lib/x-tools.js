...
export function createXSearchTool({ getContext, getConfig, runSearch, getAccessToken, onUnauthorized, defineTool }) {
  ...
  return normalizeToolParameters(defineTool({
    ...
    parameters: {
      ...
      extract_tables: { type: 'boolean', description: 'Format any benchmarks, financial data, or comparison tables from attached infographics and media into clean Markdown tables.' },
    },
    output: {
      schema: {
        type: 'object',
...
export function createThreadReaderTool({ getContext, getConfig, runSearch, getAccessToken, onUnauthorized, defineTool }) {
  if (typeof defineTool !== 'function') {
    throw new TypeError('defineTool function is required to construct x_thread_reader tool schema')
  }
  return normalizeToolParameters(defineTool({
    name: 'x_thread_reader',
    description: 'Reconstruct and read complete multi-tweet discussion threads, conversation context, and author follow-ups by tweet URL or ID.',
    parameters: {
      tweet_url_or_id: { type: 'string', required: true, description: 'Tweet URL (e.g. https://x.com/username/status/1234567890) or numeric tweet ID.' },
      include_replies: { type: 'boolean', description: 'Include notable community replies and expert rebuttals (default: true).' },
      max_depth: { type: 'number', description: 'Maximum thread depth to retrieve (default: 10, max: 25).' },
      extract_tables: { type: 'boolean', description: 'Format any data tables, benchmarks, or charts from thread media into Markdown tables.' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render(_args, value) {
        const lines = []
        if (value.tweet_id) lines.push(`Thread Target: ${value.tweet_id}${value.author ? ` (${value.author})` : ''}`)
        if (value.degraded) lines.push(`(degraded: ${value.degraded_reason || 'no citations'})`)
        if (value.answer) lines.push(String(value.answer))
        lines.push(...renderCitations(value.citations))
        return [{ type: 'text', text: lines.join('\n') || JSON.stringify(value) }]
    ...
    async execute(args, exec) {
      ...
      const rawInput = String(args.tweet_url_or_id || '').trim()
      if (!rawInput) throw new Error('tweet_url_or_id is required')

      let tweetId = ''
      let authorHandle = ''
      const urlMatch = rawInput.match(/(?:twitter\.com|x\.com)\/([a-zA-Z0-9_]+)\/status\/(\d+)/i)
      if (urlMatch) {
        authorHandle = urlMatch[1]
        tweetId = urlMatch[2]
      } else {
        const idMatch = rawInput.match(/\b(\d{8,25})\b/)
        if (idMatch) {
          tweetId = idMatch[1]
        } else {
          tweetId = rawInput
      ...
      const includeReplies = args.include_replies ?? true
      const maxDepth = Math.min(Math.max(Number(args.max_depth) || 10, 1), 25)

      let query = `Reconstruct the full discussion thread for tweet ${tweetId}`
      if (authorHandle) query += ` by author @${authorHandle}`
      query += `. Assemble the author's original root post, chronological thread continuations (up to ${maxDepth} posts), and ${includeReplies ? 'top notable community replies/rebuttals with high engagement' : 'omit unrelated chatter'}. Present the full unshortened text of each tweet in sequence.`
      ...
      const res = await runSearch({
        ...
        model: cfg.model,
        query,
        allowed_x_handles: authorHandle ? [authorHandle] : [],
        only_original_posts: false,
        only_threads: true,
        enable_image_understanding: true,
        extract_tables: Boolean(args.extract_tables),
        timeoutMs: ((Number(cfg.timeoutSeconds) || 180) + 30) * 1000,
        retries: cfg.retries,
        signal: exec?.signal,
        auto_fallback_model: cfg.autoFallbackModel,
...
export function createFindExpertsTool({ getContext, getConfig, runSearch, getAccessToken, onUnauthorized, defineTool }) {
  if (typeof defineTool !== 'function') {
    throw new TypeError('defineTool function is required to construct x_find_experts tool schema')
  }
  return normalizeToolParameters(defineTool({
    name: 'x_find_experts',
    description: 'Discover leading domain researchers, verified practitioners, and authoritative contributors on X (Twitter) by topic or technology.',
    parameters: {
      domain_or_topic: { type: 'string', required: true, description: 'Domain, research field, technology, or topic (e.g. "quantum computing", "Rust async", "LLM reasoning").' },
      min_followers: { type: 'number', description: 'Optional minimum followers threshold.' },
      language: { type: 'string', description: 'Language focus code (e.g. "en", "zh").' },
      limit: { type: 'number', description: 'Number of expert profiles to return (default: 5, max: 15).' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render(_args, value) {
        const lines = []
        if (value.domain) lines.push(`Experts in: ${value.domain}`)
        if (value.degraded) lines.push(`(degraded: ${value.degraded_reason || 'no citations'})`)
        if (value.answer) lines.push(String(value.answer))
        lines.push(...renderCitations(value.citations))
        return [{ type: 'text', text: lines.join('\n') || JSON.stringify(value) }]
    ...
    async execute(args, exec) {
      ...
      const domain = String(args.domain_or_topic || '').trim()
      if (!domain) throw new Error('domain_or_topic is required')

      const limit = Math.min(Math.max(Number(args.limit) || 5, 1), 15)
      let query = `Identify the top ${limit} most authoritative researchers, practitioners, and domain experts on X (Twitter) discussing: ${domain}.`
      if (args.language) query += ` Focus primarily on posts in language: ${args.language}.`
      if (args.min_followers) query += ` Filter for accounts with at least ${args.min_followers} followers.`
      query += ' For each expert, provide their @handle, core affiliation/background, primary discussion topics, key seminal posts/threads, and why their insights are high signal.'
