...
async function getUpdateStatus(options, target = runtime()) {
...
export function registerPluginUpdater(ctx, options) {
  ...
  if (typeof ctx?.webServer?.register !== 'function') {
    ctx?.logger?.warn?.('[dsh-grok-xsearch] webServer service unavailable, updater skipped')
    return () => {}
  }

  const endpoint = options?.endpoint || '/api/dsh-grok-xsearch/update'
  ...
  return ctx.webServer.register({
    ...
    handler: async (request, response) => {
      try {
        ...
        ctx?.logger?.warn?.(`[dsh-grok-xsearch] plugin updater failed: ${String(error)}`)
