...
const MAX_MODELS_CACHE_ENTRIES = 20

export function clearModelsCache() {
  MODELS_CACHE.clear()
}

function pruneExpiredModelsCache() {
  const now = Date.now()
  for (const [k, v] of MODELS_CACHE.entries()) {
    if (now >= v.expiresAt) {
      MODELS_CACHE.delete(k)
    }
  }
  while (MODELS_CACHE.size >= MAX_MODELS_CACHE_ENTRIES) {
    const oldest = MODELS_CACHE.keys().next().value
    if (oldest) MODELS_CACHE.delete(oldest)
    else break
  }
}
...
export async function listModelsForSettings({ accessToken, baseUrl, fetchImpl = fetch }) {
  ...
  const cacheKey = `${root}::${String(accessToken).slice(-16)}`
  ...
  try {
    ...
    const result = rows.length ? mergeModels(rows, XSEARCH_MODELS) : XSEARCH_MODELS
    pruneExpiredModelsCache()
    MODELS_CACHE.set(cacheKey, {
