...
export const Config = z.object({
  ...
  model: z.string().default('grok-4.20-reasoning'),
...
export function apply(ctx, config) {
  ...
  const syncTool = () => {
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      timeoutMs: (live().timeoutSeconds + 30) * 1000,
      async execute(args) {
        const cfg = live()
  ...
  async function configResponse() {
    return { ok: true, config: publicConfig(live()), account: await accountView(ctx) }
  }

  async function completeOAuth(code, state) {
    const row = pending.get(state)
    if (!row) throw new Error('login session expired; start Connect again')
    const cfg = live()
