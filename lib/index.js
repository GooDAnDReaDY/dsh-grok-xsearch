...
export function apply(ctx, config) {
  ...
  // ctx.get('settings') отдаёт саму службу, а не контекст: у неё register лежит
  // на верхнем уровне. Прежняя проверка обращалась к service.settings.register,
  // такого поля нет — условие всегда было ложным, и пространство настроек не
  // объявлялось вовсе. Молча: ни ошибки, ни следа в журнале. Видно это было
  // только со стороны ядра — в перечне объявленных пространств плагина не было,
  // а значит, его настройки нельзя было ни прочитать, ни записать из
  // интерфейса, и карточка во вкладке «Плагины» появиться не могла.
  ...
  if (typeof settingsService?.register === 'function') {
    ...
    // watch, а не onChange: такого метода у области настроек нет. Прежде это
    // не всплывало, потому что ветка целиком не исполнялась.
