...
.gx-collapse-btn:hover{color:var(--dsw-alias-label-primary)}`
    ...
    function Section(props) {
      ...
      if (!scopeRef.current && settingsScopeService) {
        try { scopeRef.current = settingsScopeService.bind({ namespace: NS }) } catch (_) { scopeRef.current = null }
      }
    ...
    function PluginCard(props) {
      ...
      React.useEffect(() => { ensureCss() }, [])
      return h('div', { className: 'gx-card' },
        h('div', {
          className: 'gx-card-head', onClick: () => setOpen((prev) => !prev), role: 'button', tabIndex: 0,
    ...
    function apply(ctx) {
      ...
      try {
        const injected = ctx.slots.inject('settings.plugin.item', () => ctx.slots.register(
          {
        ...
        if (!injected) {
          const warnMsg = '[dsh-grok-xsearch] slots.inject returned false for settings.plugin.item; card slot not available'
          if (typeof ctx.logger?.warn === 'function') ctx.logger.warn(warnMsg)
          else console.warn(warnMsg)
        }
      } catch (err) {
        const warnMsg = '[dsh-grok-xsearch] Failed to register settings.plugin.item slot: ' + (err?.message || err)
        if (typeof ctx.logger?.warn === 'function') ctx.logger.warn(warnMsg)
        else console.warn(warnMsg)
      }
    }
