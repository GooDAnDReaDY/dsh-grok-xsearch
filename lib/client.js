window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Шеврон карточки — та же иконка, что у карточек ядра.
     *
     * Ядро рисует IconChevronDownOutline14 из своего набора примитивов и
     * поворачивает её на 180 градусов при раскрытии. Свои символы вроде
     * треугольника рядом с ней выглядят чужеродно: у каждого плагина получался
     * свой значок, и список карточек превращался в разнобой.
     */
    ...
    /** Запасной шеврон той же формы, если набора примитивов в сборке нет. */
    ...
    const CSS =
      ...
      '.gx-card{display:flex;flex-direction:column;gap:10px;padding:12px 14px;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-layer-3)}' +
    ...
    function PluginCard() {
      ...
      return React.createElement('li', { className: 'gx-item' },
        ...
        open ? React.createElement('div', { className: 'gx-body' },
          React.createElement(Section, { compact: true }),
        ) : null,
      )
    }
