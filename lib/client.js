...
.gx-collapse-btn:hover{color:var(--dsw-alias-label-primary)}`
    ...
    function apply(ctx) {
        ...
        if (typeof ctx.slots.inject === 'function') {
          try {
            ctx.slots.inject(slotName, () => {
              try { return registerFn() } catch (err) { void err }
            })
            return
          } catch (err) { void err }
        }
        if (typeof ctx.slots.register === 'function') {
          try { registerFn() } catch (err) { void err }
        }
      }

      let moved = false
      try {
        moved = Boolean(ctx.slots?.inject?.('settings.plugin.item', () => ctx.slots.register(
          {
      ...
      registerSlotWhenReady('settings.section', () => ctx.slots.register(
        ...
        (props) => h(ErrorBoundary, null, h(Section, { ...props, ctx: (props && props.ctx) || ctx }))
      ))
    }
