...
### 5. `x_thread_reader` — Реконструкция тредов и цепочек твитов

Полная хронологическая реконструкция авторских тредов, цепочек рассуждений и ключевых реплаев по ссылке или ID твита.

| Параметр | Тип | Обязательный | Описание |
|---|---|---|---|
| `tweet_url_or_id` | `string` | **Да** | URL твита (например, `https://x.com/username/status/1234567890`) или числовой ID |
| `include_replies` | `boolean` | Нет | Включать заметные ответы сообщества и экспертные реплики (по умолчанию: `true`) |
| `max_depth` | `number` | Нет | Максимальная глубина треда (по умолчанию: `10`, максимум: `25`) |
| `extract_tables` | `boolean` | Нет | Форматировать таблицы, графики и бенчмарки из медиа в Markdown-таблицы |

---

### 6. `x_find_experts` — Поиск профильных экспертов и лидеров мнений

Поиск ведущих исследователей, разработчиков и ключевых специалистов в заданной предметной области на X (Twitter).

| Параметр | Тип | Обязательный | Описание |
|---|---|---|---|
| `domain_or_topic` | `string` | **Да** | Предметная область, тема или технология (например, `quantum computing`, `LLM reasoning`) |
| `min_followers` | `number` | Нет | Минимальное число подписчиков |
| `language` | `string` | Нет | Код языка публикаций (например, `en`, `ru`, `zh`) |
| `limit` | `number` | Нет | Количество профилей экспертов (по умолчанию: `5`, максимум: `15`) |
...
- **Версия 0.3.12 (Модуль автообновления, защита маршрутов и декомпозиция клиента)**:
  ...
  - **Строгая fail-closed защита маршрутов**: Защита мутирующих маршрутов (`/config`, `/cache/clear`, `/oauth/complete`, `/logout`, `/api/dsh-grok-xsearch/update`) проверкой loopback и origin/referer с защитой от DNS-rebinding и CSRF.
  - **Декомпозиция клиентской части**: Оптимизация и сокращение `lib/client.js` до 589 строк (ниже лимита в 600 строк) с хелперами полей ввода при полном сохранении автономности браузерного ModuleLoader.
  - **Безопасная регистрация инструментов**: Регистрация вспомогательных инструментов обёрнута в безопасный хелпер с логированием диагностики через `ctx.logger.warn`.
  - **Изоляция жизненного цикла локали**: Регистрация клиентской локали переведена в `ctx.effect` с уборщиком (disposer).
  - **Спецификация метаданных клиента**: В `package.json` заполнен `dsh.client.inject`.
