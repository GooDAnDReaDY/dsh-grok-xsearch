...
- **Do**:
  ...
  - Поддерживать исключительно канонический en и обязательный zh в коде плагина (русификация — через external dsh-russian-lang).
...
- **2026-09-10 (UI & Stability)**: Интерфейс карточки настроек приведён к единому стандарту `dsh-clinebot`: внедрён `ErrorBoundary`, структурированные секции (`.gx-section-card`), адаптивная сетка, мягкие бейджи статуса, прогресс-бар кэша ответов с кнопкой мгновенной очистки, удалён мёртвый код (`publicAccountView`, `requestOrigin`, `webCallbackUri`), защищено разрешение сервисов `credentials` и добавлены 13 дополнительных модульных тестов вспомогательных функций (итого 58 тестов).
- **2026-09-16 (v0.3.12 Remediation & Updater)**:
  - Внедрён канонический модуль автообновления lib/updater.js с конечной точкой /api/dsh-grok-xsearch/update, корректным сравнением semver с учётом пререлизов, отсутствием обхода карантина pnpm и блоком управления обновлениями в карточке настроек (#40).
  - Проведена декомпозиция и оптимизация lib/client.js: размер уменьшен с 830 до 589 строк с сохранением браузерной автономности ModuleLoader и всех 10 полей конфигурации (#45).
  - Усилена безопасность маршрутов: проверка источника в lib/http.js переведена в строгий fail-closed режим с защитой от DNS-rebinding и CSRF (#42).
  - Регистрация инструментов обёрнута в безопасный хелпер registerToolSafe с логированием предупреждений через ctx.logger.warn (#47).
  - Жизненный цикл локали изолирован через ctx.effect с диспозером (#43).
